...
Route::middleware(['auth', 'verified', 'role:coordinator'])->prefix('coordinator')->name('coordinator.')->group(function () {
    Route::get('/coordinate', [CoordinatorProjectManagementController::class, 'coordinatePage'])->name('coordinate'); // Changed path and name

    Route::put('/projects/{project}/users/{user}/toggle-status', [CoordinatorProjectManagementController::class, 'toggleUserProjectStatus'])->name('projects.users.toggle-status');
    Route::post('/projects/{project}/users/link', [CoordinatorProjectManagementController::class, 'linkUserToProject'])->name('projects.users.link');
    Route::delete('/projects/{project}/users/{user}/unlink', [CoordinatorProjectManagementController::class, 'unlinkUserFromProject'])->name('projects.users.unlink');
});
